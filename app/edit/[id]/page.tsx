...
export default async function Edit(props:editProps){
    ...
    // const [updateResults] = await db.query<RowDataPacket[]>('upadate new_schema.board SET title = ?, content = ? where id = ?', const  [title, content, id]);
  
  
//         const updateResults = await db.query<RowDataPacket[]>('upadate new_schema.board SET title = ?, content = ? where id = ?', [title, content, id]);
// }
